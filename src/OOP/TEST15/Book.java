...
public class Book {
    ...
    public static String capitalize(String word) {
        ...
        if (word.trim().isEmpty()) {
            ...
            for (int i = 0; i < s.length; i++) {
                x += s[i].substring(0, 1).toUpperCase() + s[i].substring(1).toLowerCase() + " ";
            }
            x = x.trim();
            return x;
    ...
    public static Boolean isTitleWord(String word) {
        String[] unav = {"a", "an", "the", "and", "but", "for", "nor", "or", "so", "yet"
                , "at", "by", "in", "into", "near", "of", "on", "to", "than", "via"};
        for (String each : unav) {
    ...
    public void setAuthor(String author) {
        ...
        System.out.println(capitalize(author)) ;
    ...
    public void setTitle(String title) {
        if (title.isEmpty()) {
            ...
            String result2=capitalize(all[0]);
            String result = "";
            for (int i = 1; i < all.length; i++) {
                if (!isTitleWord(all[i])) {
            ...
            System.out.println(result2+" "+(result.trim()));
            this.title=result2+" "+result.trim();
        }
    }
